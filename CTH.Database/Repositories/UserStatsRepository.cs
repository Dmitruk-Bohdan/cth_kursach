...
public class UserStatsRepository : IUserStatsRepository
{
    ...
    private readonly string _getSubjectStatisticsWithTopicsQuery;
        ...
        ILogger<UserStatsRepository> logger)
    {
        ...
        _getSubjectStatisticsWithTopicsQuery = sqlQueryProvider.GetQuery("StatisticsUseCases/Queries/GetSubjectStatisticsWithTopics");
    }
    ...
    public Task<IReadOnlyCollection<UserStats>> GetSubjectStatisticsWithTopicsAsync(long userId, long subjectId, CancellationToken cancellationToken)
    {
        var parameters = new[]
        {
            new NpgsqlParameter("user_id", NpgsqlDbType.Bigint) { Value = userId },
        ...
        return _sqlExecutor.QueryAsync(
            _getSubjectStatisticsWithTopicsQuery,
            reader => new UserStats
            {
                ...
                AverageScore = reader.IsDBNull(reader.GetOrdinal("average_score")) ? null : reader.GetDecimal(reader.GetOrdinal("average_score")),
                AverageTimeSec = reader.IsDBNull(reader.GetOrdinal("average_time_sec")) ? null : reader.GetInt32(reader.GetOrdinal("average_time_sec")),
                CreatedAt = reader.IsDBNull(reader.GetOrdinal("created_at")) ? DateTimeOffset.UtcNow : reader.GetFieldValue<DateTimeOffset>(reader.GetOrdinal("created_at")),
                UpdatedAt = reader.IsDBNull(reader.GetOrdinal("updated_at")) ? DateTimeOffset.UtcNow : reader.GetFieldValue<DateTimeOffset>(reader.GetOrdinal("updated_at")),
                Subject = reader.IsDBNull(reader.GetOrdinal("subject_id")) ? null : new Subject
                {
                    Id = reader.GetInt64(reader.GetOrdinal("subject_id")),
                    SubjectName = reader.GetString(reader.GetOrdinal("subject_name"))
